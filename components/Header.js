...
const Header = (props) => {
  ...
  if (process.browser) {
    window.addEventListener(
      ...
      debounce(() => {
        ...
        if (scroll > 0 && !isScrolling) setIsScrolling(true);
        else if (scroll === 0 && isScrolling) setIsScrolling(false);
      }, 0)
    );
  }
  ...
  if (isScrolling) classHeader += " isScrolling";
  if (menuIsOpen) classHeader += " menuIsOpenHeader";
