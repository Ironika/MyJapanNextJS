...
const CardScanVa = (props) => {
  ...
  const [isBookmarked, setIsBookmarked] = useState(
    data.isBookmarked ? data.isBookmarked : false
  );
  const { user } = useUser();
  ...
  const [selectedChapter, setSelectedChapter] = useState({
    label: data.chapt
      ? "Chapter " + data.chapt.split(" ")[1].replace(":", "")
      : "",
    value: data.chapt ? data.chapt.split(" ")[1].replace(":", "") : "",
  });
  ...
  useEffect(() => {
    const fetchDatas = async () => {
      const result = await getApiDatas(
        "scansva",
        null,
        null,
        null,
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    const reader = await getApiDatas(
      "scansva",
      null,
      null,
      null,
  ...
  const handleClickBookmark = async (e, scan) => {
    ...
    if (isBookmarked) await putApiDatas(`users/${user.id}`, datas);
    else await postApiDatas(`users/${user.id}`, datas);
  ...
  const handleClickBookmarkModal = async () => {
    ...
    newScan.link = currentLink.replace(
      /chapter-[0-9]*/gm,
      selectedChapter.label.replace("Chapter", "chapter").replace(" ", "-")
    );
  ...
  return (
    <>
      <Tilt
        className={
          props.isInProfile ? "tilt tilt-va card-profile" : "tilt tilt-va"
        }
      >
        <div className="card-scans-va" style={style} onClick={handleClick}>
          {user && (
            <i
              ...
              onClick={(e) => handleClickBookmark(e, data)}
            ></i>
          )}
          <a
            href={data.link}
            className="card-scans-va-content"
            target="_blank"
            rel="noopener noreferrer"
          >
            <p className="date">
              {props.isInProfile && data.chaptMax
                ? `${data.chapt} / ${data.chaptMax}`
                : data.chapt}
            </p>
            <h3>{data.title}</h3>
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="nav-modal">
            ...
            {user && (
              <button
                onClick={handleClickBookmarkModal}
                className="addBookmark"
                disabled={
                  isBookmarked &&
                  data.chapt.split(" ")[1] === selectedChapter.value
                    ? true
                    : false
                }
              >
