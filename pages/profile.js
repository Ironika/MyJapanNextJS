      ...
      <div className="tabs-container">
        <div className="tabs">
          <input
            ...
            id="scans"
            name="tabs"
            checked={checked === "scans"}
            onChange={() => handleChangeChecked("scans")}
          />
          <label className="tab" htmlFor="scans">
            Scans{/*<span className="notification">2</span>*/}
          </label>
          <input
            type="radio"
            id="animes"
            name="tabs"
            checked={checked === "animes"}
            onChange={() => handleChangeChecked("animes")}
          />
          <label className="tab" htmlFor="animes">
            Animes
          </label>
          <input
            type="radio"
            ...
            onChange={() => handleChangeChecked("settings")}
          />
        ...
        <div className="card-container">
          {loading
            ? fakeArray.map((item, index) => (
                <CardScansVaSkeleton key={index} />
              ))
            : scans.length > 0 &&
              scans
                ?.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt))
                ?.map((scan, key) => (
                  <CardScanVa key={key} data={scan} isInProfile={true} />
                ))}
        </div>
      )}
